...
AIRTABLE_BASE  = get_secret("AIRTABLE_BASE_ID", "app8eWcW7BHJ4EGxv")
...
def scrape_url(url):
    if not JINA_KEY:
        return None, "Jina API key not set."
...
if page == "📚 Recipe Library":
    ...
    if not ANTHROPIC_KEY:
        st.error("Anthropic API key not set. Add it to your Streamlit secrets.")
        st.stop()
    ...
    with tab1:
        ...
        if input_type == "🔗 URL":
            ...
            if url:
                source_url = url
                if not JINA_KEY:
                    st.warning("Jina API key not set — switch to 'Paste text' instead.")
                elif st.button("Fetch & parse recipe", type="primary"):
            ...
            content = st.text_area("Paste the full recipe here", height=200,
                placeholder="Ingredients, instructions, anything you have...")
            if st.button("Parse & save recipe", type="primary"):
                do_parse = True

        elif input_type == "📸 Instagram caption":
            st.info("Instagram blocks direct access — paste the caption text below.")
            ig_url = st.text_input("Instagram post URL (for reference)", placeholder="https://www.instagram.com/p/...")
            content = st.text_area("Paste the caption / recipe text here", height=200)
            if ig_url:
                source_url = ig_url
            if st.button("Parse & save recipe", type="primary"):
                do_parse = True

        if do_parse and content:
            with st.spinner("Claude is parsing the recipe..."):
                ...
                recipe.pop("Source URL", None) if not source_url else None
        ...
        elif do_parse and not content:
            st.warning("Please add recipe content first.")
...
# ── PAGE: Profile ────────────────────────────────────────────────────────────
    ...
    with st.form("profile_form"):
        ...
        c7, c8 = st.columns(2)
        default_meals = profile.get("Meals to Plan", "Breakfast,Lunch,Dinner,Snack").split(",") if profile.get("Meals to Plan") else MEAL_TYPES
        meals_selected = c7.multiselect("Meals to plan", MEAL_TYPES, default=default_meals)
        prep_opts = [0, 1, 2, 3, 7]
        prep_val = int(profile.get("Meal Prep Days", 2))
        prep_idx = prep_opts.index(prep_val) if prep_val in prep_opts else 2
        prep_days = c8.selectbox("Meal prep days per week", prep_opts, index=prep_idx)

        c9, c10 = st.columns(2)
        restrict = c9.text_input("Dietary restrictions", value=profile.get("Dietary Restrictions", ""))
...
# ── PAGE: Weekly Plan ────────────────────────────────────────────────────────
            ...
            try:
                ...
                for d in plan.get("days", []):
                    for m in d.get("meals", []):
                        ...
                        linked_id = recipe_lookup.get(m["recipe"].lower())
                        if linked_id:
                            record["Recipe"] = [linked_id]
                        else:
                            record["Notes"] = m["recipe"]
    ...
    if plan:
        ...
        tgt_cal = int(profile.get("Calories Target", 2000))
        tgt_prot = int(profile.get("Protein Target", 150))

        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Avg calories/day", f"{int(avg.get('calories', 0))}", delta=f"{int(avg.get('calories', 0)) - tgt_cal} vs target")
        m2.metric("Avg protein/day", f"{int(avg.get('protein', 0))}g", delta=f"{int(avg.get('protein', 0)) - tgt_prot}g vs target")
        m3.metric("Avg carbs/day", f"{int(avg.get('carbs', 0))}g")
        m4.metric("Est. weekly cost", f"${plan.get('estimated_weekly_cost', 0):.0f}",
                  delta=f"${plan.get('estimated_weekly_cost', 0) - float(profile.get('Weekly Budget', 100)):.0f} vs budget",
                  delta_color="inverse")
        ...
        for i, day in enumerate(DAYS):
            ...
            with cols[i]:
                ...
                for meal in day_data.get("meals", []):
                    mtype = meal["type"]
                    icon = {"Breakfast": "🌅", "Lunch": "☀️", "Dinner": "🌙", "Snack": "🍎"}.get(mtype, "•")
...
# ── PAGE: Grocery List ───────────────────────────────────────────────────────
    ...
    if total > budget:
        st.warning(f"You're ${total - budget:.2f} over budget.")
    else:
    ...
    for cat in GROCERY_CATS:
        ...
        for item in cat_items:
            ...
            c1.write(f"{item.get('Amount', '')} {item.get('Ingredient', '')}")
